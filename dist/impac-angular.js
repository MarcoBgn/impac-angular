...
angular.module('impac.services',
  [
    ...
    'impac.services.highcharts-theme',
...
$templateCache.put("widgets/sales-customers-engagement.tmpl.html","<!--\n  Component generated by Impac! Widget Generator!\n-->\n<div widget-sales-customers-engagement>\n  <!-- Settings Panel -->\n  <div ng-show=\"widget.isEditMode\" class=\"edit\">\n    <h4>Widget settings</h4>\n\n    <div setting-organizations parent-widget=\"widget\" class=\"part\" deferred=\"::orgDeferred\" />\n\n    <!-- Buttons displayed on the lower  -->\n    <div class=\"bottom-buttons\" align=\"right\">\n      <button class=\"btn btn-default\" ng-click=\"initSettings()\">Cancel</button>\n      <button class=\"btn btn-warning\" ng-click=\"updateSettings()\">Save</button>\n    </div>\n  </div>\n\n  <!-- Content Panel -->\n  <div ng-hide=\"widget.isEditMode\">\n    <!-- Data found -->\n    <div ng-show=\"isChartDisplayed\" class=\"\">\n      <!-- Chart -->\n      <div id=\"{{chartId()}}\" class=\"customers-engagement-chart\"></div>\n\n    </div>\n\n    <!-- No data found -->\n    <div ng-if=\"(isDataFound==false)\" common-data-not-found on-display-alerts=\"onDisplayAlerts()\" endpoint=\"::widget.category\" width=\"::widget.width\" />\n  </div>\n</div>\n");
...
(function () {console.info("Impac! Front-End Library - v1.6.5"); window["impac"] = {"version": "1.6.5"};}).call();
(function() {
  angular.module('impac.filters.moment-date', []).filter('momentDate', ["$translate", "ImpacTheming", function($translate, ImpacTheming) {
    return function(date, component) {
      var format, settings, validPeriods;
...
(function() {
  angular.module('impac.services.bolt-resources', []).service('BoltResources', ["$http", "ImpacMainSvc", function($http, ImpacMainSvc) {
    ...
    this.create = function(boltPath, resourcesName, attributes, relationships) {
      var body, url;
    ...
    this.destroy = function(boltPath, resourcesName, resourceId) {
      var url;
      url = [boltPath, resourcesName, resourceId].join('/');
...
(function() {
  angular.module('impac.services.highcharts-factory', []).factory('HighchartsFactory', ["$filter", "HighchartsThemeService", function($filter, HighchartsThemeService) {
    var Chart, templates, todayUTC;
    templates = {
      line: Object.freeze({
        get: function(series, options, xAxis, yAxis, plotOptions) {
    ...
    return Chart = (function() {
      ...
      Chart.prototype.render = function(data, options, renderer_callback) {
        var chartConfig;
        if (_.isObject(data)) {
          this.data = data;
        }
        ...
        if (_.isEmpty(this.hc)) {
          this.hc = Highcharts.stockChart(this.id, chartConfig, renderer_callback);
        } else {
          this.hc.update(chartConfig, renderer_callback);
        }
        return this;
      };

      Chart.prototype.template = function() {
        return this._template.get(this.data.series, this.options, this.data.xAxis, this.data.yAxis, this.data.plotOptions);
      };
...
(function() {
  angular.module('impac.services.highcharts-theme', []).service('HighchartsThemeService', function() {
    Highcharts.theme = {
  ...
  module.component('transactionsAdd', {
    ...
    controller: function() {
      ...
      ctrl.$onInit = function() {
        ...
        return ctrl.tempDate = moment().add(10, 'weeks').toDate();
      };
      ctrl.isValid = function() {
        return !_.isEmpty(ctrl.trx.name) && !isNaN(Number(ctrl.trx.amount)) && Number(ctrl.trx.amount) !== 0;
      };
      ctrl.createTransaction = function() {
        ctrl.onHide();
        return ctrl.onCreateTransaction({
          trx: ctrl.trx
        });
      };
      return ctrl;
...
(function() {
  ...
  module.component('transactionsList', {
    ...
    controller: function() {
      ...
      ctrl.changeResourcesType = function() {
        return ctrl.onChangeResources({
          resourcesType: ctrl.resourcesType
        }).then(function() {
          return ctrl.$onInit();
        });
      };
      ctrl.changeExpectedDate = function(trx) {
        trx.showReset = true;
        return ctrl.onUpdateExpectedDate({
          trxId: trx.id,
          date: trx.datePicker.date
        });
      };
      ctrl.resetExpectedDate = function(trx) {
        var expDate, m;
        trx.showReset = false;
...
(function() {
  ...
  module.controller('WidgetAccountsCashProjectionCtrl', ["$scope", "$q", "$filter", "$timeout", "ImpacKpisSvc", "ImpacWidgetsSvc", "ImpacAssets", "HighchartsFactory", "BoltResources", function($scope, $q, $filter, $timeout, ImpacKpisSvc, ImpacWidgetsSvc, ImpacAssets, HighchartsFactory, BoltResources) {
    var dateFilter, imgSrc, imgTemplate, legendFormatter, onClickBar, onZoom, settingsPromises, todayUTC, updateLocked, w, zoomMetadata;
    w = $scope.widget;
    todayUTC = moment().startOf('day').add(moment().utcOffset(), 'minutes');
    updateLocked = false;
    zoomMetadata = {};
    ...
    $scope.trxList.deleteTransaction = function(resourcesType, trxId) {
      _.remove($scope.trxList.transactions, function(trx) {
        return trx.id === trxId;
      });
    ...
    $scope.addForecastPopup.createTransaction = function(trx) {
      return BoltResources.create(w.metadata.bolt_path, this.resourcesType, {
        title: trx.name,
    ...
    w.initContext = function() {
      ...
      return BoltResources.index(w.metadata.bolt_path, 'companies', {
        metadata: _.pick(w.metadata, 'organization_ids')
      }).then(function(response) {
        return $scope.firstCompanyId = response.data.data[0].id;
      });
    };
    w.format = function() {
      var options;
...
(function() {
  ...
  module.controller('WidgetAccountsLiveBalanceCtrl', ["$scope", "$q", "ChartFormatterSvc", "$filter", function($scope, $q, ChartFormatterSvc, $filter) {
    ...
    $scope.timePeriodDeferred = $q.defer();
    $scope.histModeDeferred = $q.defer();
    settingsPromises = [$scope.orgDeferred.promise, $scope.accountBackDeferred.promise, $scope.timePeriodDeferred.promise, $scope.histModeDeferred.promise];
    $scope.isDataFound = true;
    w.initContext = function() {
      return $scope.isDataFound = w.content != null;
    };
    $scope.getName = function() {
      if (w.selectedAccount != null) {
        return w.selectedAccount.name;
    ...
    $scope.getTitle = function() {
      if (w.content != null) {
        return w.content.table.table_title;
      }
    };
    $scope.getOpeningBalance = function() {
      if (w.content != null) {
        return _.find(w.content.figure.metrics, function(metric) {
          return metric.label === 'opening';
        });
      }
    };
    ...
    $scope.getHeaders = function() {
      if (w.content != null) {
        return w.content.table.table_headers;
      }
    };
    $scope.getTotal = function() {
      var summary;
      if (w.content != null) {
        summary = _.find(w.content.table.tables, function(table) {
          return table.table_title === 'Bank Summary';
        });
      }
      if (w.selectedAccount != null) {
        return _.find(summary.table_rows, function(row) {
          var name_match;
          name_match = w.selectedAccount.name === 'All Accounts' ? 'Total' : w.selectedAccount.name;
          return row.column_1 === name_match;
        });
      }
    };
    $scope.getStatementBalance = function() {
      var statement;
      if (w.content != null) {
        statement = _.find(w.content.table.tables, function(table) {
          return table.table_title === 'Bank Statement';
        });
      }
      if (statement) {
        return _.find(statement.table_rows, function(row) {
          return row.column_2 === "Closing Balance";
        });
      }
    };
    $scope.displayAccount = function() {
  ...
  module.directive('widgetAccountsLiveBalance', function() {
    return {
      restrict: 'A',
...
(function() {
  ...
  module.controller('WidgetSalesAveragePurchaseSizeCtrl', ["$scope", "$q", "$filter", "ImpacWidgetsSvc", "ImpacAssets", "HighchartsFactory", "ImpacUtilities", function($scope, $q, $filter, ImpacWidgetsSvc, ImpacAssets, HighchartsFactory, ImpacUtilities) {
    var dateFilter, imgSrc, onClickBar, rendererCallback, settingsPromises, todayUTC, w;
    w = $scope.widget;
    ...
    imgSrc = function(name) {
      return ImpacAssets.get(_.camelCase(name + 'ArrowUp'));
    };
    $scope.chartId = function() {
      return "averagePurchaseSizeChart-" + w.id;
    };
    $scope.chartDeferred = $q.defer();
    onClickBar = function(event) {
      return $scope.isChartDisplayed = false;
    };
    rendererCallback = function(chart) {
      var point, text;
      console.log("" + (imgSrc('green')));
      point = chart.series[3].yData[0];
      text = "<h4> Insight Data </h4> <br> <h5> Previous Month: " + point + " </h5>";
      chart.renderer.image("" + (imgSrc('green')), 30, 400, 20, 20).attr({
        zIndex: 7
      }).add();
      chart.renderer.label(text, 20, 400, 'rect').css({
        color: '#E2E2E2'
      }).attr({
    ...
    $scope.onButtonBack = function() {
      return $scope.isChartDisplayed = true;
    };
    w.initContext = function() {
      return $scope.isDataFound = w.content != null;
    };
    w.format = function() {
      ...
      $scope.chart.formatters = function() {
        var currency, xAxisLabels, yAxisLabels;
        currency = this.options.currency;
        xAxisLabels = {
          labels: {
            formatter: function() {
              if (this.chart.rangeSelector.options.selected >= 3) {
                return moment.utc(this.value).format('MMM YYYY');
              } else {
        ...
        yAxisLabels = {
          labels: {
            formatter: function() {
              return $filter('mnoCurrency')(this.value, currency, false);
            }
          }
        };
        return {
  ...
  module.directive('widgetSalesAveragePurchaseSize', function() {
    return {
      restrict: 'A',
...
(function() {
  ...
  module.controller('WidgetSalesAverageVisitCtrl', ["$scope", "$q", "$filter", "ImpacWidgetsSvc", "ImpacAssets", "HighchartsFactory", "BoltResources", function($scope, $q, $filter, ImpacWidgetsSvc, ImpacAssets, HighchartsFactory, BoltResources) {
    var dateFilter, settingsPromises, todayUTC, w;
    w = $scope.widget;
    $scope.orgDeferred = $q.defer();
    settingsPromises = [$scope.orgDeferred.promise];
    todayUTC = moment().startOf('day').add(moment().utcOffset(), 'minutes');
  ...
  module.directive('widgetSalesAverageVisit', function() {
    return {
      restrict: 'A',
...
(function() {
  ...
  module.controller('WidgetSalesCustomersEngagementCtrl', ["$scope", "$q", "$filter", "ImpacWidgetsSvc", "ImpacAssets", "HighchartsFactory", "ImpacUtilities", function($scope, $q, $filter, ImpacWidgetsSvc, ImpacAssets, HighchartsFactory, ImpacUtilities) {
    var dateFilter, settingsPromises, todayUTC, w;
    w = $scope.widget;
    $scope.isChartDisplayed = true;
    $scope.orgDeferred = $q.defer();
        ...
        return {
          ...
          tooltip: {
            ...
            formatter: function() {
              ...
              date = moment.utc(this.x).format('Do MMM YYYY');
              name = this.series.name;
              return "<strong>" + date + "</strong><br>" + name + ": " + this.y + "%";
            }
          }
        };
      };
  ...
  module.directive('widgetSalesCustomersEngagement', function() {
    return {
      restrict: 'A',
...
(function() {
  ...
  module.controller('SettingBoltAccountCtrl', ["$scope", "$filter", function($scope, $filter) {
    var formatAmount, setOptions, setSelected, setting, w;
    w = $scope.parentWidget;
    setting = {};
    setting.key = "bolt-account";
    setting.isInitialized = false;
    setting.initialize = function() {
      if (w.content.settings.selectors.length !== 0) {
        w.accountList = setOptions();
      }
      w.selectedAccount = setSelected();
      if ((w.content != null) && (w.accountList != null) && (w.metadata != null) && (w.metadata.account_uid != null)) {
    ...
    setting.toMetadata = function() {
      if (w.selectedAccount != null) {
        return {
          account_uid: w.selectedAccount.account_id
        };
      }
    };
    setOptions = function(name) {
      ...
      return _.find(w.content.settings.selectors, function(selector) {
    ...
    setSelected = function(selected) {
      ...
      return _.find(w.accountList, function(acc) {
        return acc.account_id === selected;
      });
    };
    formatAmount = function(anAccount) {
      var balance;
    ...
    $scope.formatLabel = function(anAccount) {
      if (anAccount.currency != null) {
        return anAccount.name + " (" + anAccount.currency + ")";
      } else {
  ...
  module.directive('settingBoltAccount', ["$templateCache", "$translate", function($templateCache, $translate) {
    return {
      restrict: 'A',
      scope: {
        parentWidget: '=',
      ...
      link: function(scope, element) {
        if (!scope.label) {
          return scope.label = $translate.instant('impac.widget.settings.bolt-account.label');
        }
      },
...
(function() {
  ...
  module.directive('settingTimePeriod', ["$templateCache", "$q", "$log", "$timeout", "$filter", "ImpacTheming", "$translate", function($templateCache, $q, $log, $timeout, $filter, ImpacTheming, $translate) {
    return {
      restrict: 'A',
      scope: {
        parentWidget: '=',
      ...
      link: function(scope) {
        ...
        scope.formattedMinDate = function(toDate) {
          if (toDate == null) {
            toDate = void 0;
          }
          return $filter('momentDate')(scope.getMinDate(toDate), 'time-period');
        };
...
(function() {
  ...
  module.directive('settingTimeSlider', ["$templateCache", "$timeout", "ImpacMainSvc", "ImpacUtilities", "ImpacTheming", function($templateCache, $timeout, ImpacMainSvc, ImpacUtilities, ImpacTheming) {
    return {
      restrict: 'A',
      scope: {
        parentWidget: '=',
